...
class WM_CGT_MP_modal_detection_operator(bpy.types.Operator):
    ...
    def get_chain(self, stream) -> Optional[cgt_nodes.NodeChain]:
        ...
        if self.user.enum_detection_type == 'HAND':
            ...
            input_node = mp_holistic_detector.HolisticDetector(
                stream,
                self.user.holistic_model_complexity,
                self.user.min_detection_confidence,
                self.user.refine_face_landmarks,
            )
    ...
    def setup_leveling(self):
        """Prepare camera leveling and smoothing before real detection starts."""
        ...
        if self.user.detection_input_type == 'movie':
            ...
            if not ok:
                logging.warning("[cgt_leveling] Clip leveling estimate failed; continuing without correction.")
                cgt_leveling.LEVELING.reset('off')
        else:
    ...
    def get_stream(self):
        ...
        dimensions = {'sd': (720, 480), 'hd': (1240, 720), 'fhd': (1920, 1080)}
...
class WM_CGT_MP_leveling_diagnostic_operator(bpy.types.Operator):
    """Estimate camera-level correction without modifying the rig."""
    bl_label = "Leveling Diagnostic Operator"
    bl_idname = "wm.cgt_leveling_diagnostic_operator"
    bl_description = "Estimate camera leveling correction and print diagnostics."

    @classmethod
    def poll(cls, context):
        user = getattr(context.scene, "cgtinker_mediapipe", None)
        return user is not None and user.enum_detection_type in ('POSE', 'HOLISTIC') and not user.modal_active

    def execute(self, context):
        user = context.scene.cgtinker_mediapipe
        from .cgt_mp_core import cgt_leveling_prescan

        if user.detection_input_type == 'movie':
            mov_path = bpy.path.abspath(user.mov_data_path)
            if not Path(mov_path).is_file():
                self.report({'ERROR'}, f"Invalid video path: {mov_path}")
                return {'CANCELLED'}
            cgt_leveling.LEVELING.reset('fixed')
            ok = cgt_leveling_prescan.estimate_leveling_from_movie(
                mov_path,
        ...
        if not ok:
            self.report({'WARNING'}, "No pose detected during diagnostics; check the Blender console.")
            return {'CANCELLED'}

        angle = cgt_leveling.LEVELING.last_angle_deg
        self.report({'INFO'}, f"Estimated correction angle: {angle:.2f} degrees.")
        return {'FINISHED'}
